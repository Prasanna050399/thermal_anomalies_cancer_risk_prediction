...
class RemarksScreen(object):
    ...
    def retranslateUi(self, remarksWindow):
        ...
        remarksWindow.setWindowTitle(_translate("remarksWindow", "Remarks of Radiologist"))
        self.backButton.setText(_translate("remarksWindow", "Back"))
        self.label.setText(_translate("remarksWindow", "Impression by Radiologist"))
        self.nextButton.setText(_translate("remarksWindow", "Proceed to Report Generation"))
        self.label_2.setText(_translate("remarksWindow", "Left Breast Findings"))
        self.label_4.setText(_translate("remarksWindow", "Suggest"))
        self.label_3.setText(_translate("remarksWindow", "Right Breast Findings"))
    ...
    def buttonClicked(self, button):
        if button == self.backButton:
            ...
            tocontinue, entities = self.findEntitiesNotAvailable()
            if tocontinue is True:
                self.blockAll()
                msgBox = QMessageBox()
    ...
    def findEntitiesNotAvailable(self):
        tocontinue = True
        res = []
        entities = [["Front view scan image", "Front view Right ROI", "Front view Left ROI", "Normal Temperature point", "Front view Anomaly Detection image", "Front view Edge Detection image"], ["Right lateral view scan image", "Right lateral view ROI", "Right lateral view Anomaly Detection image", "Right lateral view Edge Detection image"], ["Left lateral view scan image", "Left lateral view ROI", "Left lateral view Anomaly Detection image", "Left lateral view Edge Detection image"], ["Right oblique view scan image", "Right oblique view ROI", "Right oblique view Anomaly Detection image", "Right oblique view Edge Detection image"], ["Left oblique view scan image", "Left oblique view ROI", "Left Oblique view Anomaly Detection image", "Left Oblique view Edge Detection image"]]
        for i in range(len(entities)):
            if i == 0:
                ...
                if self.cache.anomalies[i] is None:
                    res.append(entities[i][4])
                    tocontinue = False
                if self.cache.edges[i][0] is None:
                    res.append(entities[i][5])
                    tocontinue = False
            else:
                if self.cache.originalImages[i] is None:
                    res.append(entities[i][0])
    ...
    def saveCacheToFileSystem(self):
        ...
        for i in range(len(self.cache.originalImages)):
            cv.imwrite(originalNames[i], self.cache.originalImages[i])

        for i in range(len(self.cache.anomalies)):
            cv.imwrite(anomalyNames[i], self.cache.anomalies[i])

        for i in range(len(self.cache.edges)):
            cv.imwrite(edgeNames[i], self.cache.edges[i][0])
...
class MasterThread:
    ...
    def saveReportDone(self):
        ...
        self.visitSaveThread = None
        import os
        #os.system("start " + self.savefilename)
        os.startfile(self.savefilename)
        self.endWork()
...
class PredictionThread(QObject):
    ...
    def doWork(self):
        ...
        #res = ["Negative Breast Mass Characteristics", "82.45"] ##
...
class ReportGenerationThread(QObject):
    ...
    def doWork(self):
        ...
        obj = ReportGeneration(self.cache, self.savefilename)
