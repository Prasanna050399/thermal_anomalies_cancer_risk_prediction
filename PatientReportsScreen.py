...
class PatientReportsScreen(object):
    ...
    def blockAll(self):
        ...
        self.viewReportButton.setEnabled(False)
    ...
    def unblockAll(self):
        ...
        self.viewReportButton.setEnabled(True)
        ...
        if button == self.backButton:
            ...
            if idx != -1:
                ...
                if _ is True:
                    ...
                    self.cache.time = visitTime
                    temp = con.getPatientVisitDataFromDB(self.cache.id, self.cache.date, self.cache.time)
                    otherCache = con.getPatientDataForCache(self.cache.id)
                    con.closeConnection()
                    self.cache.id = temp["PatientID"]
                    self.cache.date = temp["VisitDate"]
                    self.cache.time = temp["VisitTime"]
                    self.cache.originalImages = temp["OriginalImages"]
                    self.cache.anomalies = temp["AnomalyImages"]
                    # adding patient info for report generation
                    self.cache.name = otherCache["Name"]
                    self.cache.ref = otherCache["ReferedBy"]
                    self.cache.age = otherCache["Age"]
                    self.cache.specialRemarks = otherCache["SpecialRemarks"]
                    self.cache.historyMeterScore = otherCache["HistoryMeterScore"]
                    #self.cache.edges = temp["EdgeImages"] ###
                    for i in range(len(self.cache.edges)):
                        self.cache.edges[i][0] = temp["EdgeImages"][i]
                    self.cache.diagnosticScore = temp["DiagnosticScore"]
                    self.cache.remarks = temp["Remarks"]
                    #self.cache.asymmetryImage = temp["AsymmetryImage"] ##
                    self.cache.nn = temp["NN"]
                    '''
                        #Report generation code
                    '''
                    # testing code
                    #print(dumps(temp, cls=NumpyEncoder))
                    print(self.cache.print())
                    print("Patient visit data loaded into memory successfully.")
                    #self.cache.saveCacheToFileSystem() ##
                    #self.cache.visualize()
                    #print("Patient visit data visualized successfully.")
                    from tempfile import gettempdir
                    temp_directory = gettempdir()
                    current_path = temp_directory + "\\" + "report_1.pdf"
                    obj = ReportGeneration(self.cache, current_path)
                    import os
                    os.startfile(current_path)
                    #
                    #subprocess.call([current_path])
                    self.unblockAll()
                else:
                    self.unblockAll()
                    failureMsgBox = QMessageBox()
